...
describe('FhirVerifier', () => {
  ...
  before(async () => {
    ...
    // Load circuit files
    wasmFile = path.join(projectRoot, 'circuits/out/fhirverifier/FhirVerifier_js/FhirVerifier.wasm');
    provingKeyPath = path.join(projectRoot, 'circuits/out/fhirverifier/FhirVerifier_0001.zkey');
    const verificationKeyPath = path.join(projectRoot, 'circuits/out/fhirverifier/verification_key_FhirVerifier.json');
