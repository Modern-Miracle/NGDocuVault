...
OUT_DIR="${PROJECT_ROOT}/circuits/out/ageverifier"
...
if [ ! -f "${PROJECT_ROOT}/circuits/__test__/ageverifier/simple-test.js" ]; then
    echo -e "${YELLOW}Warning: simple-test.js not found at ${PROJECT_ROOT}/circuits/__test__/ageverifier/simple-test.js. Skipping original JavaScript test.${NC}"
    JS_TEST_STATUS=0
else
    echo -e "${BLUE}Running simple-test.js...${NC}"
    
    # Pass the output directory to the test
    OUT_DIR=$OUT_DIR node circuits/__test__/ageverifier/simple-test.js
    JS_TEST_STATUS=$?
...
TS_SIMPLE_TEST="${PROJECT_ROOT}/circuits/__test__/ageverifier/AgeVerifierSimpleTest.ts"
...
if [ ! -f "$TS_SIMPLE_TEST" ]; then
    ...
    OUT_DIR=$OUT_DIR npx ts-node ageverifier/AgeVerifierSimpleTest.ts
    TS_SIMPLE_STATUS=$?
...
TS_DETAILED_TEST="${PROJECT_ROOT}/circuits/__test__/ageverifier/AgeVerifierDetailedTest.ts"
...
if [ ! -f "$TS_DETAILED_TEST" ]; then
    ...
    OUT_DIR=$OUT_DIR npx ts-node ageverifier/AgeVerifierDetailedTest.ts
    TS_DETAILED_STATUS=$?
...
MOCHA_TEST="${PROJECT_ROOT}/circuits/__test__/ageverifier/AgeVerifierMochaTest.ts"
...
if [ ! -f "$MOCHA_TEST" ]; then
    ...
    OUT_DIR=$OUT_DIR npx mocha --require ts-node/register 'ageverifier/AgeVerifierMochaTest.ts' --timeout 30000
    MOCHA_STATUS=$?
