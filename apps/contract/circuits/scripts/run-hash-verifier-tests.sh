...
run_test "hashverifier/HashVerifierTest.ts" "HashVerifier"
HASH_VERIFIER_STATUS=$?

# Run other test files if they exist
if [ -f "${PROJECT_ROOT}/circuits/__test__/hashverifier/HashVerifierTest1.ts" ]; then
    run_test "hashverifier/HashVerifierTest1.ts" "HashVerifier Additional"
    if [ $? -ne 0 ]; then
