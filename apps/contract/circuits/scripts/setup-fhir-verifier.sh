...
echo -e "${BLUE}=== FHIR Verifier Circuit Setup Script ===${NC}"
...
OUT_DIR="${CIRCUITS_DIR}/out/fhirverifier"
PTAU_DIR="${CIRCUITS_DIR}/ptau"
mkdir -p ${OUT_DIR}/FhirVerifier_js

# Step 1: Compile the circuit
echo -e "${BLUE}Step 1: Compiling FHIR Verifier circuit...${NC}"
cd ${CIRCUITS_DIR}
circom FhirVerifier.circom --r1cs --wasm --sym --c -o ${OUT_DIR} -l ./__test__/node_modules
...
mv ${OUT_DIR}/FhirVerifier_js/FhirVerifier.wasm ${OUT_DIR}/FhirVerifier_js/
...
cd ${OUT_DIR}
snarkjs groth16 setup FhirVerifier.r1cs ${PTAU_FILE} FhirVerifier_0000.zkey
snarkjs zkey contribute FhirVerifier_0000.zkey FhirVerifier_0001.zkey --name="1st Contributor" -v -e="more random entropy"

# Step 5: Export the verification key
echo -e "${BLUE}Step 5: Exporting verification key...${NC}"
snarkjs zkey export verificationkey FhirVerifier_0001.zkey verification_key_FhirVerifier.json
...
if [ "$SETUP_SUCCESS" = false ]; then
    ...
    echo -e "\n${GREEN}You can now run the FHIR Verifier tests using:${NC}"
    echo -e "${YELLOW}./scripts/run-all-fhir-verifier-tests.sh${NC}"
fi 
