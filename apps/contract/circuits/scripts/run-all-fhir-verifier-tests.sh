...
echo -e "${BLUE}=== FhirVerifier Circuit Comprehensive Test Suite ====${NC}"
...
OUT_DIR="${PROJECT_ROOT}/circuits/out/fhirverifier"
WASM_FILE="${OUT_DIR}/FhirVerifier_js/FhirVerifier.wasm"
ZKEY_FILE="${OUT_DIR}/FhirVerifier_0001.zkey"
VKEY_FILE="${OUT_DIR}/verification_key_FhirVerifier.json"
...
if [ ! -f "$WASM_FILE" ] || [ ! -f "$ZKEY_FILE" ] || [ ! -f "$VKEY_FILE" ] || [ ! -f "$PTAU_FILE" ]; then
    ...
    echo -e "${YELLOW}./circuits/scripts/setup-fhir-verifier.sh${NC}"
    exit 1
fi
...
TS_SIMPLE_TEST="${PROJECT_ROOT}/circuits/__test__/fhirverifier/FhirVerifierSimpleTest.ts"
...
if [ ! -f "$TS_SIMPLE_TEST" ]; then
    ...
    OUT_DIR=$OUT_DIR npx ts-node fhirverifier/FhirVerifierSimpleTest.ts
    TS_SIMPLE_STATUS=$?
...
TS_DETAILED_TEST="${PROJECT_ROOT}/circuits/__test__/fhirverifier/FhirVerifierDetailedTest.ts"
...
if [ ! -f "$TS_DETAILED_TEST" ]; then
    ...
    OUT_DIR=$OUT_DIR npx ts-node fhirverifier/FhirVerifierDetailedTest.ts
    TS_DETAILED_STATUS=$?
...
TS_STANDARD_TEST="${PROJECT_ROOT}/circuits/__test__/fhirverifier/FhirVerifierTest.ts"
...
if [ ! -f "$TS_STANDARD_TEST" ]; then
    ...
    OUT_DIR=$OUT_DIR npx ts-node fhirverifier/FhirVerifierTest.ts
    TS_STANDARD_STATUS=$?
...
MOCHA_TEST="${PROJECT_ROOT}/circuits/__test__/fhirverifier/FhirVerifierMochaTest.ts"
...
if [ ! -f "$MOCHA_TEST" ]; then
    ...
    OUT_DIR=$OUT_DIR npx mocha --require ts-node/register 'fhirverifier/FhirVerifierMochaTest.ts' --timeout 60000
    MOCHA_STATUS=$?
...
# Modified to preserve verification_key files
echo -e "${BLUE}Cleaning up temporary test files...${NC}"
rm -f ${OUT_DIR}/*.wtns 
# Delete JSON files except verification key
find ${OUT_DIR} -name "*.json" -not -name "verification_key_*.json" -delete
