...
echo -e "${BLUE}=== FhirVerifier Circuit Tests ===${NC}"
...
if [ ! -f "$WASM_FILE" ]; then
    ...
    echo -e "${YELLOW}./scripts/setup-fhir-verifier.sh${NC}"
    exit 1
fi
...
# Run FhirVerifier tests
FHIR_VERIFIER_STATUS=0
run_test "fhirverifier/FhirVerifierTest.ts" "FhirVerifier"
FHIR_VERIFIER_STATUS=$?

# Run other test files if they exist
if [ -f "${PROJECT_ROOT}/circuits/__test__/fhirverifier/FhirVerifierDetailedTest.ts" ]; then
    run_test "fhirverifier/FhirVerifierDetailedTest.ts" "FhirVerifier Detailed"
    if [ $? -ne 0 ]; then
        FHIR_VERIFIER_STATUS=1
    fi
fi
...
echo -e "\nOverall Status:"
if [ $FHIR_VERIFIER_STATUS -eq 0 ]; then
    echo -e "${GREEN}✅ ALL FHIR VERIFIER TESTS PASSED SUCCESSFULLY! ✅${NC}"
    echo -e "${GREEN}The FhirVerifier circuit correctly handles all cases:${NC}"
    echo -e "${GREEN}1. Success - valid inputs and resource hash match${NC}"
    echo -e "${GREEN}2. Resource type verification - validates resource types correctly${NC}"
