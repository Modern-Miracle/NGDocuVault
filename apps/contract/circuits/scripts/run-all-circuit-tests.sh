...
npx ts-node --transpile-only __test__/ageverifier/AgeVerifierTest.ts
age_verifier_test_result=$?

echo -e "\n${BLUE}=== Test 2: AgeVerifierTest1.ts ===${NC}"
npx ts-node --transpile-only __test__/ageverifier/AgeVerifierTest1.ts
age_verifier_test1_result=$?

echo -e "\n${BLUE}=== Test 3: AgeVerifier.test.ts (Mocha) ===${NC}"
npx mocha -r ts-node/register __test__/ageverifier/AgeVerifier.test.ts
age_verifier_mocha_result=$?

# Run HashVerifier test
echo -e "\n${BLUE}=== Running HashVerifier Tests ===${NC}"
npx ts-node --transpile-only __test__/hashverifier/HashVerifierTest.ts
