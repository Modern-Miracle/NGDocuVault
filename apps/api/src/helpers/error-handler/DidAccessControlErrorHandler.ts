...
export class DidAccessControlErrorHandler extends BaseErrorHandler {
   ...
   */
  public decodeUnauthorizedDidError(data: string): {
    args: Record<string, any>;
    message: string;
  } {
    try {
      const args = this.decodeKnownError(
        'DidAccessControl__UnauthorizedDID',
        data
      );
      return {
        args,
        message:
          'Unauthorized DID: The DID does not have permission to perform this action'
      };
    } catch (error) {
      throw new Error(
        `Failed to decode DidAccessControl__UnauthorizedDID error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  public decodeMissingRequirementError(data: string): {
    ...
    try {
      const args = this.decodeKnownError(
        'DidAccessControl__MissingRequirement',
        data
      );
      return {
        args,
        message:
          'Missing requirement: The DID does not meet the requirements for the role'
      };
    } catch (error) {
      throw new Error(
        `Failed to decode DidAccessControl__MissingRequirement error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
