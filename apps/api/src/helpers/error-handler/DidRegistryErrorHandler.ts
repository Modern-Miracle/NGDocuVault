...
export class DidRegistryErrorHandler extends BaseErrorHandler {
  ...
  public decodeDidNotFoundError(data: string): { args: Record<string, any>; message: string } {
    try {
      ...
      throw new Error(`Failed to decode DidRegistry__DidNotFound error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
  ...
  public decodeUnauthorizedError(data: string): { args: Record<string, any>; message: string } {
    try {
      ...
      throw new Error(`Failed to decode DidRegistry__Unauthorized error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
  ...
  public decodeDidDeactivatedError(data: string): { args: Record<string, any>; message: string } {
    try {
      ...
      throw new Error(`Failed to decode DidRegistry__DidDeactivated error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
