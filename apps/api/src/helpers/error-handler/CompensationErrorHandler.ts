...
export class CompensationErrorHandler extends BaseErrorHandler {
  ...
  public decodeInsufficientBalanceError(data: string): {
    ...
    try {
      ...
      throw new Error(`Failed to decode Compensation__InsufficientBalance error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
  ...
  public decodeTokenTransferFailedError(data: string): {
    ...
    try {
      ...
      throw new Error(`Failed to decode Compensation__TokenTransferFailed error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
