...
export class TokenErrorHandler extends BaseErrorHandler {
  ...
  public decodeInsufficientBalanceError(data: string): {
    ...
    try {
      ...
      throw new Error(`Failed to decode ERC20InsufficientBalance error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
  ...
  public decodeInsufficientAllowanceError(data: string): {
    ...
    try {
      ...
      throw new Error(`Failed to decode ERC20InsufficientAllowance error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
