...
export class ZKPErrorHandler extends BaseErrorHandler {
  ...
  public decodeInvalidProofError(data: string): { args: Record<string, any>; message: string } {
    try {
      ...
      throw new Error(`Failed to decode InvalidProof error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
  ...
  public decodeVerificationFailedError(data: string): {
    ...
    try {
      ...
      throw new Error(`Failed to decode VerificationFailed error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
