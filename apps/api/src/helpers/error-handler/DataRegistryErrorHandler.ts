...
export class DataRegistryErrorHandler extends BaseErrorHandler {
   ...
   */
  public decodeRecordNotFoundError(data: string): {
    ...
    try {
      ...
      throw new Error(
        `Failed to decode DataRegistry__RecordNotFound error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns The decoded error arguments and a user-friendly message
   */
  public decodeUnauthorizedError(data: string): {
    args: Record<string, any>;
    message: string;
  } {
    try {
      ...
      throw new Error(
        `Failed to decode DataRegistry__Unauthorized error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns The decoded error arguments and a user-friendly message
   */
  public decodeAccessDeniedError(data: string): {
    args: Record<string, any>;
    message: string;
  } {
    try {
      ...
      throw new Error(
        `Failed to decode DataRegistry__AccessDenied error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
