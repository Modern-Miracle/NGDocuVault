...
export class DidAuthErrorHandler extends BaseErrorHandler {
  ...
  public decodeUnauthorizedError(data: string): { args: Record<string, any>; message: string } {
    try {
      ...
      throw new Error(`Failed to decode DidAuth__Unauthorized error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
  ...
  public decodeInvalidCredentialError(data: string): {
    ...
    try {
      ...
      throw new Error(`Failed to decode DidAuth__InvalidCredential error: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
  ...
  public decodeCredentialVerificationFailedError(data: string): {
    ...
    try {
      ...
      throw new Error(
        `Failed to decode DidAuth__CredentialVerificationFailed error: ${(error instanceof Error ? error.message : String(error))}`
      );
    }
  }
