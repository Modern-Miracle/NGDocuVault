...
const ERROR_PATTERNS: Record<string, CommonErrorTypes> = {
  ...
  'replacement transaction underpriced':
    CommonErrorTypes.REPLACEMENT_UNDERPRICED,
...
export abstract class BaseErrorHandler {
    ...
    try {
      ...
      if (error.data && this.contract) {
        try {
          ...
          const errorDescription = this.contract.interface.parseError(
            error.data
          );
  ...
  public decodeKnownError(
    ...
    try {
      ...
      throw new Error(
        `Failed to decode error ${errorName}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  protected abstract formatErrorMessage(
    errorName: string,
    args: Record<string, any>
  ): string;
