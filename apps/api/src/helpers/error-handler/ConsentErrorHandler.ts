...
export class ConsentErrorHandler extends BaseErrorHandler {
   ...
   */
  public decodeAlreadyGrantedError(data: string): {
    args: Record<string, any>;
    message: string;
  } {
    try {
      const args = this.decodeKnownError(
        'ConsentManagement__AlreadyGranted',
        data
      );
      return {
        args,
        message:
          'Consent already granted: Cannot grant consent that is already active'
      };
    } catch (error) {
      throw new Error(
        `Failed to decode ConsentManagement__AlreadyGranted error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns The decoded error arguments and a user-friendly message
   */
  public decodeAlreadyRevokedError(data: string): {
    args: Record<string, any>;
    message: string;
  } {
    try {
      const args = this.decodeKnownError(
        'ConsentManagement__AlreadyRevoked',
        data
      );
      return {
        args,
        message:
          'Consent already revoked: Cannot revoke consent that is already revoked'
      };
    } catch (error) {
      throw new Error(
        `Failed to decode ConsentManagement__AlreadyRevoked error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
