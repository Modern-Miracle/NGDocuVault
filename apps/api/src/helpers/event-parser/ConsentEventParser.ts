...
export class ConsentEventParser extends BaseEventParser {
  ...
  public decodeConsentGrantedEvent(data: string, topics: string[]): Record<string, any> {
    try {
      ...
      throw new Error(`Failed to decode ConsentGranted event: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
  ...
  public decodeConsentRevokedEvent(data: string, topics: string[]): Record<string, any> {
    try {
      ...
      throw new Error(`Failed to decode ConsentRevoked event: ${(error instanceof Error ? error.message : String(error))}`);
    }
  }
