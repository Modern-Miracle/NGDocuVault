...
export class CompensationEventParser extends BaseEventParser {
   ...
   */
  public decodePaymentProcessedEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      ...
      throw new Error(
        `Failed to decode PaymentProcessed event: ${error instanceof Error ? (error instanceof Error ? error.message : String(error)) : 'Unknown error'}`
      );
    }
  }
   ...
   * @returns The decoded event arguments
   */
  public decodeProducerPaidEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      ...
      throw new Error(
        `Failed to decode ProducerPaid event: ${error instanceof Error ? (error instanceof Error ? error.message : String(error)) : 'Unknown error'}`
      );
    }
  }
   ...
   * @returns Processed event data
   */
  private processServiceFeeWithdrawnEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
   ...
   * @returns Processed event data
   */
  private processServiceFeeChangedEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
   ...
   * @returns Processed event data
   */
  private processPaymentProcessedEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
   ...
   * @returns Processed event data
   */
  private processUnitPriceChangedEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
   ...
   * @returns Processed event data
   */
  private processOwnershipTransferredEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
