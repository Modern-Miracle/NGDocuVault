...
export class TokenEventParser extends BaseEventParser {
  ...
  public decodeTransferEvent(
    ...
    try {
      ...
      throw new Error(
        `Failed to decode Transfer event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  public decodeApprovalEvent(
    ...
    try {
      ...
      throw new Error(
        `Failed to decode Approval event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
