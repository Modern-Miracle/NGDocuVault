...
export abstract class BaseEventParser {
  ...
  public decodeKnownEvent(
    ...
    try {
      ...
      const result = this.contract.interface.decodeEventLog(
        eventFragment,
        data,
        topics
      );
      ...
      throw new Error(
        `Failed to decode event ${eventName}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  public formatTransactionReceipt(
