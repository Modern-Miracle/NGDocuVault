...
export class DidRegistryEventParser extends BaseEventParser {
   ...
   */
  public decodeDidRegisteredEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      ...
      throw new Error(
        `Failed to decode DidRegistered event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns The decoded event arguments
   */
  public decodeDidDeactivatedEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      ...
      throw new Error(
        `Failed to decode DidDeactivated event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns The decoded event arguments
   */
  public decodeDidControllerChangedEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      ...
      throw new Error(
        `Failed to decode DidControllerChanged event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns Processed event data
   */
  private processDidControllerChangedEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
   ...
   * @returns Processed event data
   */
  private processDidMetadataChangedEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
