...
export class DidIssuerEventParser extends BaseEventParser {
   ...
   */
  public decodeCredentialIssuedEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      ...
      throw new Error(
        `Failed to decode CredentialIssued event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  private processCredentialIssuedEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
