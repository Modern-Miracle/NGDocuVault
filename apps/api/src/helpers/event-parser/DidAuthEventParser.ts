...
export class DidAuthEventParser extends BaseEventParser {
   ...
   */
  public decodeAuthenticationSuccessfulEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      const args = this.decodeKnownEvent(
      ...
      throw new Error(
        `Failed to decode AuthenticationSuccessful event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns The decoded event arguments
   */
  public decodeAuthenticationFailedEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      ...
      throw new Error(
        `Failed to decode AuthenticationFailed event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   */
  private processAuthenticationSuccessfulEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
   ...
   * @returns Processed event data
   */
  private processAuthenticationFailedEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
   ...
   * @returns Processed event data
   */
  private processCredentialVerifiedEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
   ...
   * @returns Processed event data
   */
  private processCredentialVerificationFailedEvent(
    event: ParsedEvent
  ): Record<string, any> {
    return {
      ...event.args,
