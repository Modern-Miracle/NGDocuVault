...
export class ZKPEventParser extends BaseEventParser {
   ...
   */
  public decodeProofVerifiedEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      ...
      throw new Error(
        `Failed to decode ProofVerified event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
   ...
   * @returns The decoded event arguments
   */
  public decodeProofRejectedEvent(
    data: string,
    topics: string[]
  ): Record<string, any> {
    try {
      ...
      throw new Error(
        `Failed to decode ProofRejected event: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
