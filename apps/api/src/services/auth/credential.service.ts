...
export class CredentialService {
   ...
   */
  constructor() {
    this.didIssuerService = new DidIssuerService();
  ...
  public async issueCredential(
    ...
    try {
      ...
      const isIssuerActive =
        await this.didRegistryService.isDidActive(issuerDid);
      const isSubjectActive =
        await this.didRegistryService.isDidActive(subjectDid);
      ...
      const signature = await wallet.signMessage(
        ethers.toUtf8Bytes(credentialString)
      );
  ...
  public async verifyCredential(credential: any): Promise<boolean> {
    try {
      ...
      return await this.didVerifierService.verifyCredential(
        credentialType,
        issuerDid,
        subjectDid
      );
  ...
  public async revokeCredential(
    ...
    try {
      return {
        success: false,
        message:
          'Credential revocation is not supported by the current contract implementation'
      };
    } catch (error) {
   ...
   */
  public async isIssuerTrusted(
    credentialType: string,
    issuerDid: string
  ): Promise<boolean> {
    console.log(
      `Checking if issuer ${issuerDid} is trusted for credential type ${credentialType}`
    );

    try {
      return await this.didVerifierService.isIssuerTrusted(
        credentialType,
        issuerDid
      );
    } catch (error) {
  ...
  public async setIssuerTrustStatus(
    ...
    console.log(
      `Setting trust status for issuer ${issuerDid} to ${trusted} for credential type ${credentialType}`
    );

    try {
      return await this.didVerifierService.setIssuerTrustStatus(
        credentialType,
        issuerDid,
        trusted
      );
