...
import { VerifiableCredential, VerificationResults } from '../../types';
import { DocuVaultService, DidRegistryService } from '../contract';
...
export class DidResolverService {
  ...
  private docuVaultService?: DocuVaultService;
   ...
   * @param docuVaultService Optional DocuVault service for additional data lookups.
   */
  constructor() {
    this.didRegistryService = new DidRegistryService();
    this.docuVaultService = new DocuVaultService();
  }
  ...
  public async resolve(did: string): Promise<DidResolutionResult> {
    try {
      ...
      try {
        didDocument = JSON.parse(documentString) as DidDocument;
      } catch (error) {
  ...
  public async verifyCredential(
    ...
    try {
      ...
      const verificationMethod =
        issuerDidResolution.didDocument.verificationMethod?.find(
      ...
      if (!issuerAddress) {
        throw new Error(
          `Failed to extract address from issuer DID: ${issuerDid}`
        );
      }
  ...
  public async getPublicKey(did: string): Promise<string> {
    const publicKey = await this.didRegistryService.getDidDocument(did);
    return publicKey;
  }
