...
import { DocuVaultService } from '../contract';
import { AsymmetricCryptoService } from '../crypto';
import { EncryptionService } from '../crypto/symmetricEnc.service';
...
export class DataAccessControllerService {
  constructor(
    ...
    private readonly docuVaultService: DocuVaultService,
  ...
  async validateConsumerAccess(cid: string, address: string): Promise<boolean> {
    try {
      ...
      const hasAccess = await this.docuVaultService.checkAccess(cid, address);
   ...
   */
  async encryptSharedSecretForConsumer(
    secret: string,
    publicKey: string
  ): Promise<string> {
    try {
      // Encrypt the shared secret using the consumer's public key
      return this.asymmetricCryptoService.encryptWithPublicKey(
        secret,
        publicKey
      );
    } catch (error) {
