...
import {
  ...
  ContractType
} from '../../helpers/ContractHandlerFactory';
...
import { DidAuthABI } from '../../constants';
...
export class DidAuthService {
  ...
  public readonly PRODUCER_ROLE = ethers.keccak256(
    ethers.toUtf8Bytes('PRODUCER')
  );
  public readonly CONSUMER_ROLE = ethers.keccak256(
    ethers.toUtf8Bytes('CONSUMER')
  );
  public readonly PROVIDER_ROLE = ethers.keccak256(
    ethers.toUtf8Bytes('PROVIDER')
  );
  public readonly ISSUER_ROLE = ethers.keccak256(ethers.toUtf8Bytes('ISSUER'));
  public readonly OPERATOR_ROLE = ethers.keccak256(
    ethers.toUtf8Bytes('OPERATOR')
  );
  public readonly VERIFIER_ROLE = ethers.keccak256(
    ethers.toUtf8Bytes('VERIFIER')
  );
  ...
  async verifyCredentialForAction(
    ...
    try {
      return await this.contract.verifyCredentialForAction(
        did,
        credentialType,
        credentialId
      );
  ...
  async hasRequiredRolesAndCredentials(
    ...
    try {
      return await this.contract.hasRequiredRolesAndCredentials(
        did,
        roles,
        credentialIds
      );
  ...
  async setTrustedIssuer(
    ...
    try {
      const tx = await this.contract.setTrustedIssuer(
        credentialType,
        issuer,
        trusted
      );
