...
import { DidVerifierABI } from '../../constants';
...
export class DidVerifierService {
  ...
  constructor() {
    this.contract = new Contract(
      config.didVerifierContractAddress,
      DidVerifierABI,
      signer
    );
  ...
  async verifyCredential(
    ...
    try {
      return await this.contract.verifyCredential(
        credentialType,
        issuer,
        subject
      );
  ...
  async setIssuerTrustStatus(
    ...
    try {
      const tx = await this.contract.setIssuerTrustStatus(
        credentialType,
        issuer,
        trusted
      );
