...
export class EncryptionService {
  ...
  constructor(masterKeyHex?: string) {
    if (masterKeyHex) {
      ...
      if (this.masterKey.length !== this.keyLength) {
        throw new Error(
          `Invalid master key length. Expected ${this.keyLength} bytes.`
        );
      }
    } else {
  ...
  encryptWithKey(data: Buffer | string, keyHex: string): string {
    try {
      ...
      // @ts-ignore - TypeScript has issues with crypto types but this is correct
      ...
      // @ts-ignore - This is valid for GCM mode
  ...
  decryptBinary(encryptedData: Uint8Array, key: Buffer): Uint8Array {
    try {
      ...
      const tagBuffer = dataBuffer.subarray(
        this.ivLength,
