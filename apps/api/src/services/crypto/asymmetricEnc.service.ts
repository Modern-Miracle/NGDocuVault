...
export class AsymmetricCryptoService {
  ...
  public encryptWithPublicKey(data: string, publicKey: string): string {
    try {
      if (this.curveType !== CurveType.SECP256K1 || !this.ecCurve) {
        throw new Error(
          'Encryption with public key is only supported with secp256k1 curve'
        );
      }
      ...
      const sharedSecret = ephemeralKeyPair
        .derive(recipientPublicKey.getPublic())
        .toString(16);
  ...
  public decryptWithPrivateKey(
    ...
    try {
      if (this.curveType !== CurveType.SECP256K1 || !this.ecCurve) {
        throw new Error(
          'Decryption with private key is only supported with secp256k1 curve'
        );
      }
      ...
      const ephemeralKey = this.ecCurve.keyFromPublic(
        ephemeralPublicKey,
        'hex'
      );
      const sharedSecret = recipientKeyPair
        .derive(ephemeralKey.getPublic())
        .toString(16);
  ...
  public encryptLargeData(
    ...
    try {
      if (this.curveType !== CurveType.SECP256K1) {
        throw new Error(
          'Hybrid encryption is only supported with secp256k1 curve'
        );
      }
  ...
  public decryptLargeData(
    ...
    try {
      if (this.curveType !== CurveType.SECP256K1) {
        throw new Error(
          'Hybrid decryption is only supported with secp256k1 curve'
        );
      }

      // Decrypt the symmetric key
      const symmetricKeyHex = this.decryptWithPrivateKey(
        encryptedSymmetricKey,
        privateKey
      );
  ...
  public encryptBinaryData(
    ...
    try {
      if (this.curveType !== CurveType.SECP256K1) {
        throw new Error(
          'Binary encryption is only supported with secp256k1 curve'
        );
      }
  ...
  public decryptBinaryData(
    ...
    try {
      if (this.curveType !== CurveType.SECP256K1) {
        throw new Error(
          'Binary decryption is only supported with secp256k1 curve'
        );
      }
