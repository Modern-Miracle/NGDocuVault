...
export class JWTService {
  ...
  constructor(
    ...
    if (
      ...
      console.log(
        'WARNING: Using default JWT secrets in production environment'
      );
    }
  }
  ...
  public async invalidateToken(
    ...
    console.log(`Invalidating token for address: ${normalizedAddress}`);

    // Verify the token
    const payload = this.verifyAccessToken(token);

    if (!payload) {
      console.log(`Invalid token for address: ${normalizedAddress}`);
    ...
    if (payload.sub.toLowerCase() !== normalizedAddress) {
      console.log(`Token subject doesn't match address: ${normalizedAddress}`);
    ...
    try {
      ...
      console.log(
        `Successfully invalidated token for address: ${normalizedAddress}`
      );
  ...
  public generateTokens(
    ...
    const expiresInHours =
      parseInt(this.accessTokenExpiry.replace(/[^0-9]/g, ''), 10) || 1;
