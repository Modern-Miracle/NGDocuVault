...
async function loadWeb3Storage(): Promise<any> {
  try {
    return await import('@web3-storage/w3up-client');
  } catch (err) {
    console.error('Failed to load @web3-storage/w3up-client:', err);
    throw err;
  }
}
export { loadHelia, loadStrings, loadJson, loadUnixfs, loadWeb3Storage };
