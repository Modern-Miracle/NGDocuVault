...
import {
  ...
  createDid,
  getDidForAddress,
} from '@/components/user-flow/actions';

import { isAuthenticated as checkServerAuthentication } from '../utils';
import { ApiError } from '@/components/user-flow/utils/error';
import { getRedirectUrl } from '@/components/user-flow/utils/client';
import { Role, User } from '@/components/user-flow/types';
import { useAddAdmin, useRegisterHolder, useRegisterIssuer, useRegisterVerifier } from '@/hooks/use-docu-vault';

import { toast } from 'sonner';
import { UserRegistrationParam } from '@/components/user-flow/types';
import { Address } from 'viem';
import { useGetPublicKey } from '@/hooks/use-did-registry';
...
export function useServerAuth() {
  ...
  const { mutate: registerHolder } = useRegisterHolder();
  ...
  const register = useCallback(async () => {
    try {
      ...
      const params: UserRegistrationParam = {
        ...
        role: 'HOLDER_ROLE' as Role,
      };

      await registerHolder(params);
    } catch (error) {
  ...
  const refreshTokenSilently = useCallback(async () => {
    ...
    try {
      ...
      const expiresIn = user.expiresIn || 3600; // seconds
  ...
  const login = useCallback(
    async (redirectAfterLogin: boolean = true, defaultRedirect: string = '/') => {
      if (!isConnected || !address) {
        setAuthState((prev) => ({
          ...prev,
